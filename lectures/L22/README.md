# L22 - Konstruktion av timerkretsar (del I)
...
**c)** Lägg till en signal (via en vippa) döpt `timer_enable`, som togglas vid fallande flank på `button_n`.
...
* När timern får timeout ska en utsignal döpt `timeout` ettställas, samtidigt som timern ska nollställas. 
...
* När utsignalen ur respektive XNOR-grind är hög har timern räknat upp till `10` och `timeout` ska då ettställas.

**e)** Se till att lysdioden uppdateras som tänkt:
* Se till att lysdioden togglas varje gång `timeout` är hög. 
* Se till att lysdioden släcks vid nästa klockpuls om `timer_enable` är låg.
